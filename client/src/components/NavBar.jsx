    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 text-zinc-900 dark:text-white">
          ...
          >
            JCS <span className="text-[#9F2B68]">SHOP</span>
          </button>
          ...
          <div className="hidden items-center gap-6 md:flex">
            {navLinks.map(link => (
              <button
                ...
                className={`cursor-pointer text-sm font-semibold transition hover:text-[#9F2B68] ${
                  location.pathname === link.href? 'text-[#9F2B68]' : ''
                }`}
              >
                {link.label}
              </button>
            ...
            >
              ...
              Bag
              {cartCount > 0 && (
                <span className="absolute -right-2 -top-2 flex h-5 w-5 items-center justify-center rounded-full bg-[#9F2B68] text-xs font-bold text-white">
            ...
            >
              {navLinks.map(link => (
                <button
                  key={link.href}
                  onClick={() => handleLinkClick(link.href)}
                  className="font-semibold transition hover:text-[#9F2B68]"
                >
                  {link.label}
                </button>
              ))}
