...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="group relative">
      ...
      >
        <Heart className={`h-4 w-4 ${isLiked ? 'fill-[#9F2B68] text-[#9F2B68]' : 'text-zinc-700 dark:text-zinc-300'}`} />
      </button>
      ...
      <div className="mt-4">
        ...
        <h3
          onClick={() => navigate(`/product/${id}`)}
          className="mt-1 cursor-pointer text-sm font-semibold hover:text-[#9F2B68] line-clamp-1"
        >
          {name}
        </h3>
