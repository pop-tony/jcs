...
const statusConfig = {
  ...
  'processing': { label: 'Processing Order', color: 'bg-orange-500', icon: '📦', desc: 'Picking & packing your pieces', step: 2 },
  'shipped': { label: 'Shipped', color: 'bg-indigo-500', icon: '🚚', desc: 'Your order is on the way', step: 3 },
  'delivered': { label: 'Delivered', color: 'bg-green-500', icon: '✅', desc: 'Enjoy your new MIZ J pieces!', step: 4 },
  ...
  const requestNotificationPermission = async () => {
    try {
      ...
      if (permission === 'granted') {
        new Notification('MIZ J BOUTIQUE', {
          ...
          setOrder(prev => {
            if (prev?.status!== newOrder.status && prev?.status) {
              ...
              if (notifPermission === 'granted') {
                new Notification('MIZ J BOUTIQUE Order Update', {
    ...
    <div className="min-h-screen bg-white px-4 py-24 text-zinc-900 dark:bg-black dark:text-white">
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex items-center justify-between">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-sm font-semibold text-zinc-600 transition hover:text-[#9F2B68] dark:text-zinc-400"
          >
            ← Back to Orders
          </button>
        ...
        <div className="overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-zinc-900">
                ...
                return (
                  <React.Fragment key={step.key}>
                    <div className="flex flex-col items-center">
                      ...
                      } ${isCurrent? 'ring-4 ring-[#9F2B68]/20 dark:ring-[#9F2B68]/30 animate-pulse' : ''}`}>
                        {isActive? '✓' : idx + 1}
                      </div>
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="mb-8 flex gap-4 rounded-2xl bg-zinc-50 p-4 dark:bg-zinc-800">
              ...
              <div className="text-right">
                <p className="text-2xl font-bold text-[#9F2B68]">₵{order.total}</p>
              </div>
            </div>
            ...
            <div className="mt-6 rounded-2xl bg-gradient-to-br from-[#9F2B68]/10 to-[#C9376E]/10 p-6 dark:from-[#9F2B68]/20 dark:to-[#C9376E]/20">
              <div className="flex items-center justify-between">
                <span className="text-lg font-semibold text-zinc-700 dark:text-zinc-300">Total Paid</span>
                <span className="text-3xl font-bold text-[#9F2B68] dark:text-[#C9376E]">₵{order.total}</span>
              </div>
            </div>
          </div>
