...
// Boutique categories
const categories = ['All', 'Dresses', 'Tops', 'Bottoms', 'Sets', 'Accessories', 'Shoes', 'New Arrivals'];
...
export default function ShopPage() {
  ...
  // Boutique sections for "All" view
  const sections = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        category: 'Dresses',
        products: allProducts.filter(p => p.category === 'Dresses')
      },
      {
        title: "Elevated Tops",
        category: 'Tops',
        products: allProducts.filter(p => p.category === 'Tops')
      },
      {
        title: "Chic Bottoms",
        category: 'Bottoms',
        products: allProducts.filter(p => p.category === 'Bottoms')
      },
      {
        title: "Co-Ord Sets",
        category: 'Sets',
        products: allProducts.filter(p => p.category === 'Sets')
      },
      {
        title: "Accessories",
      ...
      },
      {
        title: "Shoes",
        category: 'Shoes',
        products: allProducts.filter(p => p.category === 'Shoes')
      }
        ...
        <div className="mb-12 flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-end">
          <div>
            <h1 className="mb-2 text-4xl font-black tracking-tight md:text-5xl">
              {isSearching? 'Search Results' : activeCategory!== 'All'? activeCategory : 'MIZ J Collection'}
            </h1>
            <p className="text-zinc-600 dark:text-zinc-400">
              {isSearching
               ? `Results for "${debouncedSearch}"`
                : activeCategory!== 'All'
               ? `${filtered.length} pieces in ${activeCategory}`
                : 'Curated fashion for the modern muse'}
            </p>
            {isFiltered && filtered.length > 0 && (
              <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-500">{filtered.length} pieces found</p>
            )}
          </div>

          {/* Search */}
          <div className="w-full lg:w-96">
            <div className="flex rounded-full border-2 border-zinc-300 bg-zinc-50 focus-within:border-[#9F2B68] dark:border-zinc-700 dark:bg-zinc-900">
              <input
                type="text"
                value={searchInput}
          ...
          <>
            {filtered.length === 0? (
              <div className="py-32 text-center">
                <p className="mb-4 text-xl text-zinc-500 dark:text-zinc-400">No pieces found</p>
                <button
                  onClick={clearSearch}
                  className="text-sm font-semibold text-[#9F2B68] hover:underline"
                >
                  Clear all filters
                </button>
              </div>
                  ...
                  <div className="mt-8 text-center">
                    ...
                    >
                      {showAll['filtered']? (
                        ...
                        <>Show All {filtered.length} Pieces <ChevronDown className="h-4 w-4" /></>
                      )}
                    </button>
                  </div>
                )}
                  ...
                  <div className="mb-6 flex items-end justify-between">
                    ...
                    {hasMore && (
                      <button
                        ...
                        className="flex items-center gap-2 text-sm font-bold hover:text-[#9F2B68]"
                      >
                        {isExpanded? (
                          <>Show Less <ChevronUp className="h-4 w-4" /></>
                        ) : (
